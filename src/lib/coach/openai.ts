...
const CORE_RULES = [
  ...
  "Fill usedSignals ONLY with keys from this exact list, verbatim and upper-case, and only the ones you actually relied on: GOAL, ACTIVE_PLAN, RECENT_RUNS, PLAN_ADHERENCE, CONSISTENCY, SLEEP, WEATHER, ANALYSED_RUN, RUNNER_QUESTION, HEALTH_CONTEXT, SAFETY_DECISION, COACH_MEMORY. These are keys, not prose: do not translate them, do not rephrase them, and never put a condition name, a health note, or the runner's own words in this field. Anything not on the list is discarded and the runner loses that line of provenance.",
  "Fill dataGaps ONLY with keys from this exact list, verbatim and upper-case: NO_RECENT_RUNS, NO_RECENT_PACE, NO_SLEEP_LOGGED, NO_TARGET_RACE, NO_WEATHER_DATA, NO_HEALTH_DETAILS, NO_SYMPTOM_DETAILS, NO_INJURY_DETAILS. Same rules as usedSignals — keys, not prose, never a condition name. Leave it empty when you had what you needed. Never invent weather, race, injury, sleep, or performance facts to fill a gap.",
