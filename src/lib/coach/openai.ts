...
export const COACH_PROMPT_VERSION = "coach-v12-2026-08-08";
...
const CORE_RULES = [
  ...
  "Fill usedSignals ONLY with values from this exact list, in English, lower-case, and only the ones you actually relied on: 'goal', 'active plan', 'recent runs', 'recent pace', 'adherence', 'consistency', 'sleep', 'weather', 'analysed run', 'chronic condition', 'runner question', 'safety decision', 'coach memory'. Do not invent other values, do not translate them, and do not list signals you did not use — the server translates this list into the runner's language, so anything off-list reaches the runner in English.",
  "Fill dataGaps ONLY with values from this exact list, in English, lower-case: 'no recent runs', 'no recent pace', 'no sleep logged', 'no target race', 'no weather data', 'no heart-condition details', 'no symptom details', 'no injury details'. Same rule as usedSignals: do not translate them, the server does that. Leave it empty when you had what you needed. Never invent weather, race, injury, sleep, or performance facts to fill a gap — say the data is missing instead.",
