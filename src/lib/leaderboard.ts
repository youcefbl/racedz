  ...
  const [pace, distance] = await Promise.all([
    prisma.$queryRaw<LeaderboardEntry[]>`
      SELECT * FROM (
        ...
        WHERE r."isPublic" = true AND r."validity" = 'VALID' AND u."profilePrivate" = false AND r."distanceKm" >= ${MIN_PACE_DISTANCE_KM}
          ${wilayaCondition}
          ${monthCondition}
    ...
    prisma.$queryRaw<LeaderboardEntry[]>`
      SELECT * FROM (
        ...
        FROM "RunnerRun" r
        INNER JOIN "User" u ON u."id" = r."userId"
        WHERE r."isPublic" = true AND r."validity" = 'VALID' AND u."profilePrivate" = false
          ${wilayaCondition}
          ${monthCondition}
