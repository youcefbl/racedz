...
import { ArrowRight, BrainCircuit, CalendarCheck2, KeyRound, UserRound } from "lucide-react";
      ...
      <div className="mx-auto grid min-h-[calc(100vh-8rem)] max-w-6xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_420px] lg:items-center lg:px-8">
        <section className="relative isolate order-2 flex flex-col justify-center gap-7 overflow-hidden rounded-2xl bg-gradient-to-br from-brand-teal via-[#0c5650] to-[#0a3a36] p-8 text-white shadow-soft rz-hide-native sm:p-10 lg:order-1">
          ...
          <div className="relative space-y-4">
            ...
            <h1 className="text-balance text-3xl font-black leading-tight sm:text-4xl">{t.loginHeadline}</h1>
            <p className="max-w-md text-base leading-7 text-teal-50">{t.loginSub}</p>
          </div>
          ...
          <ul className="relative grid gap-3 sm:grid-cols-3">
            <LoginFeature icon={UserRound} title={t.featRunnersTitle} text={t.featRunnersText} />
            <LoginFeature icon={KeyRound} title={t.featOrganizersTitle} text={t.featOrganizersText} />
            <LoginFeature icon={CalendarCheck2} title={t.featRaceDaysTitle} text={t.featRaceDaysText} />
          </ul>
        </section>
