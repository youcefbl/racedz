...
/** A GPX file for one run. Generous: a long ultra with 1 Hz sampling is still well under this. */
...
export async function POST(request: Request) {
  ...
  let form: FormData;
  try {
    form = await readBoundedFormData(request, MAX_GPX_BYTES + MULTIPART_OVERHEAD_BYTES);
  } catch (error) {
    if (error instanceof BodyTooLargeError) {
      return NextResponse.json({ error: "That GPX file is too large (max 5 MB)." }, { status: 413 });
    }
  ...
  if (file.size > MAX_GPX_BYTES) return NextResponse.json({ error: "That GPX file is too large (max 5 MB)." }, { status: 413 });
