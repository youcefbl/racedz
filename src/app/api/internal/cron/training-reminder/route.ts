...
// Scheduled job: remind runners who have a run planned today not to skip it. There is no in-process
// scheduler, so an external trigger (host crontab, a cron container, or any uptime-cron service)
// should POST here once each morning — schedule it for local morning in Africa/Algiers:
//
//   curl -fsS -X POST https://zidrun.com/api/internal/cron/training-reminder \
//     -H "Authorization: Bearer $CRON_SECRET"
//
// Guarded by CRON_SECRET; constant-time compare and a hard refusal when the secret is unset so the
// endpoint is never world-open by accident. Idempotent per day, so re-runs won't double-remind.
...
export async function POST(request: Request) {
  ...
  try {
    const result = await remindTodaysWorkouts();
