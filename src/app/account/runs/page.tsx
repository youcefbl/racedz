...
export default async function RunsPage({ searchParams }: { searchParams?: Promise<{ lang?: string }> }) {
  ...
  const { runs, analyzedRuns, weightKg } = await getRunsScreenData(session.user.id, 50);
  const initialRuns = JSON.parse(JSON.stringify(runs)) as CoachRun[];

  return <RunsView initialRuns={initialRuns} analyzedRuns={analyzedRuns} weightKg={weightKg} locale={locale} />;
}
