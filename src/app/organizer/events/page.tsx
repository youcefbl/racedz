      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-gray-950">Events</h1>
        ...
        {params?.created ? (
          <div className="mb-5 rounded-lg border border-green-100 bg-green-50 p-4 text-sm font-semibold text-green-700">
            Event submitted for admin review. It will stay hidden from the public site until approved.
          </div>
        ) : null}

        {races.length === 0 ? (
          <div className="rounded-lg border border-gray-200 bg-white p-8 text-center shadow-sm">
            <h2 className="text-xl font-black text-gray-950">No events yet</h2>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-gray-600">
              Create your first race and submit it for RaceDZ admin review.
            </p>
            <ButtonLink href="/organizer/events/new" className="mt-5">
              Create Event
            </ButtonLink>
          </div>
        ) : (
                ...
                <article key={race.id} className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                  <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-start">
                    <div className="space-y-4">
                      ...
                      <div>
                        <Link href={`/organizer/events/${race.id}`} className="text-xl font-black text-gray-950 transition hover:text-brand-teal">
                          {race.title}
                        </Link>
                    ...
                    <div className="grid gap-2 sm:grid-cols-2 lg:w-44 lg:grid-cols-1">
                      <ButtonLink href={`/organizer/events/${race.id}`} variant="outline" size="sm">
                        Overview
                      </ButtonLink>
                      <ButtonLink href={`/organizer/events/${race.id}/registrations`} variant="secondary" size="sm">
                        Registrations
                      </ButtonLink>
                    </div>
                  </div>
