...
| 3 | P0 | **Run signed physical-device acceptance on 2026-07-30** (`PR-050`, `RUN-001`–`RUN-005`) | Engineering + owner | The exact signed candidate passes auth, deep links/back navigation, safe areas, Groups, voice test, guided warm-up/work/cool-down, background GPS, pause/resume, force-kill restore, logout/account switching, sustained non-foot auto-pause, stationary-start drift, Google Drive/local GPX import, GPS cold-start/map settling, compact run charts, pending-finish recovery, mobile input focus, and immediate saved-run visibility. Record device/OS/build and results in this file. |
