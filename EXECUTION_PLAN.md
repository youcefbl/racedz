...
| 2026-07-30 | Production passive scan (`SEC-001`/`SEC-005`/`SEC-006`, per docs/SECURITY_ATTACK_TEST_PLAN.md Phase 2/3) | Ran the plan's production-sanctioned passive checks only (no active scan/brute force/load) against `https://zidrun.com`: dockerized `testssl.sh` and OWASP ZAP baseline (`zap-baseline.py`). testssl.sh: **A+ (96/100)**, no vulnerable protocol/cipher (Heartbleed/POODLE/BEAST/CRIME/FREAK/DROWN/LOGJAM/RC4 all clear), valid cert, HSTS with preload. ZAP baseline: 52 PASS, 0 FAIL, 15 WARN — reviewed each: `X-Powered-By` leak already known (fix committed, awaiting deploy); CSP `img-src https:` wildcard is an intentional documented tradeoff; "Absence of Anti-CSRF Tokens" is a scanner false positive (Next.js Server Actions use origin-based CSRF, not a hidden form field, which the scanner can't see — worth explicitly testing a cross-origin POST for `SEC-005` rather than trusting the scanner's silence); "User Controllable HTML Attribute" on `?lang=` confirmed as the `lang` attribute, value is enum-validated to en/fr/ar only, not exploitable. Fixed two real findings: `racedz-locale` cookie was missing `Secure` (added, conditional on HTTPS so local dev still works; not `HttpOnly` since client JS deliberately reads/writes it) and Caddy's `/uploads/*` block was missing `Strict-Transport-Security` (added). Commit `ba0e5cd`. Raw scan output kept outside the repo per the plan's evidence-handling rule. The rest of the plan (active ZAP, Phases 4-7) still needs the isolated staging stack running — `stg.zidrun.com` doesn't resolve yet, and `gitleaks`/`semgrep`/`trivy`/`testssl.sh`-as-a-standalone-tool/`k6` aren't installed anywhere yet; only `nmap`/`docker`/`adb`/`apkanalyzer` are available today. |
