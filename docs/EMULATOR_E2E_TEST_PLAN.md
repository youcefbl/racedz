...
| 4.18 | **Rapid-fire "Skip step" during a guided workout** (the original incident trigger) | All 14 steps advance cleanly through to "Workout complete", no crash | `use-workout-guidance.ts` bounds-safety regression |
| 4.19 | **Sign out with an active/paused recording**, sign in as a **different** account, then sign back in as the original | Confirm dialog on sign-out; the second account sees no trace of the first account's run; the original account's run is intact on return | Cross-account leakage (`run-store.ts` per-user keys) |
| 4.20 | Sustained non-foot speed (e.g. simulated ~10 m/s movement) for 2+ minutes while tracking | Auto-pauses, shows "doesn't look like it was on foot" | Needs *consecutive* qualifying segments — see evidence note below on why this is hard to reproduce via `adb emu geo fix`/`geo nmea` |

**2026-07-28 evidence (Pixel 8 / Android emulator, debug APK against local `dev:lan`):**
- **4.18 passed.** Started an Intervals guided workout, tapped "Skip step" 20× as fast as `adb shell input tap` would issue them. All 14 steps advanced correctly to "Workout complete" — no crash, no error boundary, no console/logcat exceptions.
- **4.14 passed.** Force-stopped the app mid-tracking (`am force-stop`, confirmed the foreground-service notification was gone) and relaunched: session persisted, `/account/runs` rendered cleanly (no crash), the route/map restored intact as paused with correct elapsed-time math.
- **4.19 passed.** Signed out of `runner@example.com` with a paused recording — got the expected confirm dialog, and the visible delay before the redirect to `/login` confirmed the native teardown is genuinely awaited, not fire-and-forget. Logged in as `admin@zidrun.com` (a different account/role): no crash, no sign of the other account's run. Logged back in as `runner@example.com`: the original paused run was still there, untouched, same route/stats as before the switch.
- **4.20 inconclusive, not a confirmed defect.** Fed ~140 seconds of simulated ~10 m/s movement two ways (`adb emu geo fix` in a loop, and `adb emu geo nmea` with an explicit GPRMC speed field) — both produced the correct live "doesn't look like a run on foot" warning (confirms `detectNonFootActivity` fires), but the rolling 120-consecutive-second auto-pause did not visibly trigger either time (recorder still showed "Pause", not "Resume"). `adb logcat` showed `PersonalSafety: Reject: Too frequent location with insignificant accuracy improvement` during both attempts — an Android system-level location-throttling service rejecting rapid mock-location updates, which plausibly breaks the *consecutive*-segment assumption the 15s-gap-reset logic depends on. The underlying pure function (`advanceHighSpeedWindow` in `run-lifecycle.ts`) is deterministically unit-tested and passes (`test:run-incident`); this is recorded as an emulator/mock-location fidelity limitation, not a verified app bug — a real device with a genuine moving GPS track (e.g. driving with the app open) is the only way to close this out with confidence.
